...
export class EditBusComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.busForm = this.fb.group({
      ...
      busRoute: ['', [Validators.required, Validators.minLength(3), Validators.maxLength(50)]],
      busCategory: ['', [Validators.required, Validators.minLength(3), Validators.maxLength(15)]],
  ...
  deleteBus(): void {
    if(this.bus.id === 0) {
      ...
      if (confirm(`Do you want to delete the bus: ${this.bus.busName}?`)) {
        this.bookingService.deleteBus(this.bus.id)
          .subscribe({
  ...
  saveBus(): void {
    if (this.busForm.valid) {
      if (this.busForm.dirty) {
        ...
        if (bus.id === 0) {
          this.submitted = true;
          this.bookingService.createBus(bus)
            .subscribe({
              next: () => this.onSaveComplete(),
              error: err => this.errorMessage = err
            });
        } else {
          this.bookingService.updateBus(bus)
            .subscribe({
              next: () => this.onSaveComplete(),
              error: err => this.errorMessage = err
          });
